...
# Estilos visuales para interfaz en móvil y desktop
st.markdown("""
<style>
...
LOCAL_CATALOG_FILE = "todos_amiibos.json"
USER_DATA_FILE = "mi_coleccion_smash.json"
...
def cargar_amiibos_smash_local():
    if not os.path.exists(LOCAL_CATALOG_FILE):
        st.error(f"No se encontró el archivo '{LOCAL_CATALOG_FILE}'. Colócalo en la misma carpeta del proyecto.")
        return []
    ...
    for raw_id, info in amiibos_dict.items():
        # Limpieza de ID hexadecimal
        clean_id = raw_id.lower().replace("0x", "")
        
        # En el estándar de 16 caracteres hexadecimales:
        # clean_id[0:8] = head, clean_id[8:16] = tail
        # clean_id[12:14] representa la serie del amiibo ('00' = Super Smash Bros.)
        if len(clean_id) == 16:
            ...
            if series_code == "00":
                ...
                # Obtener la franquicia de origen mediante los primeros 3 caracteres hex
                game_code = f"0x{clean_id[0:3]}"
                game_name = game_series_dict.get(game_code, "Smash Bros.")

                # URL oficial de la imagen por CDN de GitHub
...
with col_search:
    busqueda = st.text_input("🔍 Buscar por personaje o franquicia...", placeholder="Ej: Mario, Link, Ryu...")
